...
// captions.windows — the SAME array the karaoke style reads. Without this the
// "tela dividida" style parks the karaoke caption on the seam and leaves the
// stacked one at its default offset, which the split then re-frames straight
// onto the speaker's mouth. A window may state `stackedOffsetY` directly; if it
// only carries the karaoke `paddingBottom`, that is converted so one array can
// serve both styles.
type Win = {start: number; end: number; paddingBottom?: number; stackedOffsetY?: number};
type CapCfg = {
...
const Cue: React.FC<{cue: CueData; cueDurationFrames: number}> = ({cue, cueDurationFrames}) => {
  ...
  // Resolved PER FRAME, not per cue: a cue that starts before a window and runs
  // into it has to move mid-cue, or it stays parked while the layout changes
  // underneath it. `frame` is local to this cue's Sequence, so rebuild the
  // absolute one. Compared in FRAMES and with the same `+1` video lag the
  // karaoke shell and the split layout use — an epsilon test on seconds, or a
  // missing lag, lands the move one frame off the layout it exists to follow.
  ...
  const offsetY = win
    ? win.stackedOffsetY ??
      (win.paddingBottom != null
        ? (height - win.paddingBottom - height / 2) / height
        : OFFSET_Y)
    : OFFSET_Y;
  const baseY = Math.round(height * offsetY);
