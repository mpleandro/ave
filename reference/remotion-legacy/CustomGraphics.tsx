...
type SplitInsert = {
  ...
  bandH?: number;
  layout?: 'top' | 'bottom';
  // Per-window override of LAYOUT[layout]. A single focusY only holds when the
  // subject keeps one position for the whole video, which a multi-take cut rarely
  // does — measured on one real edit the hair-top moved ~170px between takes, and
  // any single value either cropped the high takes or left a dead gap under the
  // seam on the low ones. Set it per window, and set it from the frame where the
  // head is HIGHEST inside that window (the worst case), not from an average.
  // Do not derive it from the transform on paper: render that worst frame, crop
  // from the seam down, and read the headroom off a grid. ~50-60px reads right.
  focusY?: number;
  zoom?: number;
};
...
const SplitFrame: React.FC<{
  ...
  focusY?: number;
  zoom?: number;
}> = ({image, bandH, fit, progress, layout, focusY: focusYIn, zoom: zoomIn}) => {
  // slow Ken-Burns so the band is not a dead still
  const artScale = 1 + 0.03 * progress;
  const zoom = zoomIn ?? LAYOUT[layout].zoom;
  const focusY = focusYIn ?? LAYOUT[layout].focusY;
